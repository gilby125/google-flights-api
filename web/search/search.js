...
async function loadAirports() {
    try {
        ...
        document.getElementById('destination').setAttribute('list', 'destinationList');
...
async function loadPriceHistory(origin, destination) {
    try {
        const response = await fetch(`${ENDPOINTS.PRICE_HISTORY}?origin=${origin}&destination=${destination}`);
        if (!response.ok) {
...
async function handleSearch(event) {
    ...
    const travelClass = document.getElementById('class') ? document.getElementById('class').value : 'economy';
    const stops = document.getElementById('stops') ? document.getElementById('stops').value : 'any';
    ...
    if (elements.advancedOptions && elements.advancedOptions.style.display === 'block') {
...
function createFlightCard(offer) {
    ...
    if (!googleFlightsUrl) {
        ...
        googleFlightsUrl = createGoogleFlightsUrl(departureSegment.departure_airport, departureSegment.arrival_airport, departureTime, tripType);
    }
